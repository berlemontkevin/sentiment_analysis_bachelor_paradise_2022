...
st.title('gabby Sentiment Analysis')
...
st.markdown('Let us first look at the evolution of the sentiment of the tweets about gabby thruogh the show:')



fig_gabby = px.line(df_sentiments, x=df_sentiments.index, y=df_sentiments.columns, markers = True, title = 'Proportion of negative tweets about gabby', labels = {'index':'Date', 'value':'Proportion of negative tweets', 'variable':'Candidate'})
st.plotly_chart(fig_gabby)

fig_gabby_numbers = px.line(df_numbers, x=df_numbers.index, y=df_numbers.columns, markers = True, title = 'Number of tweets about gabby', labels = {'value': 'Number of tweets', 'index': 'Date', 'variable': 'Candidate'})
st.plotly_chart(fig_gabby_numbers)


st.markdown('### Let us now look at the most frequent words used in the tweets about gabby:')
